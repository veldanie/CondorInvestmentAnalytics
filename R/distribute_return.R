...
distribute_return <- function(ticker, series_list, reported_returns, since_date, complete_dates = TRUE){

  if(complete_dates){
    series <- complete_all_dates(ticker, series_list)[paste0(since_date, '/')]
  }else{
  ...
  return(output[!duplicated(index(output)), ])
