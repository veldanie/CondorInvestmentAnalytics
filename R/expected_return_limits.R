...
exp_ret_limits <- function(series_list, assets, ports, ref_currency, conn, asset_data, period = 'monthly', quant = 0.95, ia = FALSE) {
  ...
  mean_asset_rets_ub <- round(mean_asset_rets + qnorm(quant) * sd_asset_rets / sqrt(length(temp_returns)), 3)
  mean_asset_rets_lb <- round(mean_asset_rets - qnorm(quant) * sd_asset_rets / sqrt(length(temp_returns)), 3)
  ...
  for(port in ports){
    ...
    port_ret_ub <- round(port_return + qnorm(quant) * port_sd / sqrt(length(series_port)), 3)
    port_ret_lb <- round(port_return - qnorm(quant) * port_sd / sqrt(length(series_port)), 3)
