...
exp_limit_fun <- function(f_const, ind_rel, pos_asset, ind_dur_rest=FALSE, w_adj=rep(1, ncol(f_const))) {
  if(any(ind_rel) && !any(ind_dur_rest)){
    ...
    function(w){
      w_rest <- as.numeric(f_const %*% w)
      w_rest[ind_rel] <- (pos_asset %*% w) /w_rest[ind_rel]
      w_rest[ind_dur_rest] <- as.numeric(f_const %*% (w*w_adj))[ind_dur_rest]/((f_const[ind_dur_rest,]>0) %*% (w*w_adj))
      return(w_rest)
    }
  }else if(!any(ind_rel) && any(ind_dur_rest)){
    function(w){
      w_rest <- as.numeric(f_const %*% (w*w_adj))
      w_rest[ind_dur_rest] <- w_rest[ind_dur_rest]/((f_const[ind_dur_rest,]>0) %*% (w*w_adj))
      return(w_rest)
    }
  }else{
