...
#' @param dd_horiz Dradown horizon.
#' @param dd_quant Quantile for max drawdown.
#' @return Objective function.
#' @export

utility_fun_de <- function(type = 'absolute', mu, Sigma, lambda, w_bench = NULL, lb = rep(0, length(mu)), ub = rep(1, length(mu)), lb_act = rep(0, length(mu)), ub_act = rep(1, length(mu)), min_var = FALSE, vol_obj = Inf, f_const = rep(1, length(mu)), f_const_lb = 0, f_const_ub = Inf, same_assets_bench = TRUE, Sigma_bench = NULL, dd_obj = Inf, series = NULL, dd_horiz = '12M', dd_quant = 0.9) {
  if(type == 'absolute'){
    if (min_var == TRUE){
      function(w){
      ...
      function(w) {
        if (sum(w)!=1){w <- w/sum(w)}
        if(any(w<lb) || any(w>ub) || as.numeric(sqrt(t(w) %*% Sigma %*% w))>vol_obj || any((f_const %*% w) < f_const_lb) || any((f_const %*% w) > f_const_ub) || drawdown(series, w, dd_horiz, dd_quant)$cond_dd > dd_obj) {return(Inf)}
    ...
    if(same_assets_bench){
      function(w_act){
        ...
        if(any(w_act<lb_act) || any(w_act>ub_act) || any((w_act + w_bench) < lb) || any((w_act + w_bench)>ub) || as.numeric(sqrt(t(w_act) %*% Sigma %*% w_act))>vol_obj || any((f_const %*% (w_act+w_bench)) < f_const_lb) || any((f_const %*% (w_act+w_bench)) > f_const_ub) || drawdown(series, w_act+w_bench, dd_horiz, dd_quant)$cond_dd > dd_obj) {return(Inf)}
      ...
      function(w) {
        if (sum(w)!=1){w <- w/sum(w)}
        if(any(w<lb) || any(w>ub) || (as.numeric(sqrt(t(w) %*% Sigma %*% w)) - as.numeric(sqrt(t(w_bench) %*% Sigma_bench %*% w_bench)))>vol_obj || any((f_const %*% w) < f_const_lb) || any((f_const %*% w) > f_const_ub) || drawdown(series, w, dd_horiz, dd_quant)$cond_dd > dd_obj) {return(Inf)}
        util <- -(t(w) %*% mu - 0.5 * lambda * (!is.finite(vol_obj)) * t(w) %*% Sigma %*% w)
        return(as.numeric(util))
      }
    }
