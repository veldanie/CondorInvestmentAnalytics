...
utility_fun_de <- function(type = 'absolute', mu, Sigma, lambda, w_bench = NULL, lb = rep(0, length(mu)), ub = rep(1, length(mu)), lb_act = rep(0, length(mu)), ub_act = rep(1, length(mu)), min_var = FALSE, vol_obj = Inf, f_const = rep(1, length(mu)), f_const_lb = 0, f_const_ub = Inf) {
  if(type == 'absolute'){
    if (min_var == TRUE){
      function(w){
